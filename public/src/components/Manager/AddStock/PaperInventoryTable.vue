<template>
  <v-data-table
    ...
    :headers="headers"
    :items="$firestore.collections.stock.filter(x => x.type === 'Paper')"
  ...
  >
    ...
    <template slot="footer">
      <td colspan="100%">
        <strong>Paper Total Today: {{ this.totalPaper }}</strong>
      </td>
    </template>
  </v-data-table>
</template>
...
export default {
  ...
  created () {
    let stockWeights = []
    this.$firestore.collections.stock.filter(x => x.type === 'Paper').map(item => stockWeights.push(parseInt(item.weight)))
    this.totalPaper = stockWeights.reduce((a, b) => a + b, 0)
